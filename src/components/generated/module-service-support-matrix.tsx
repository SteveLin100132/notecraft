...
type Column = { id: ColumnId; label: string; system: string; subtitle?: string };
...
const COLUMNS: Column[] = [
  ...
  { id: 'bpm',   label: '業務流程管理（BPM）', system: 'BPM', subtitle: '派案・諮詢・報價・執案' },
...
const MATRIX: Matrix = {
  'inhouse':    {
    ...
    clm:   { level: 'partial', footnote: 1,    adopted: false },
...
const FOOTNOTES: Footnote[] = [
  { id: 1, text: '自建系統 × 合約管理：可管理合約資料，但無法取代實體合約、解決電子簽章法律效力 → 故搭配點點簽' },
  { id: 2, text: 'Vital CRM × 業務流程／合約管理：須整合 BizForm 才能建表單並交付節點資料' },
  { id: 3, text: 'Vital CRM × 看板：統計範圍僅含 CRM 自身資料' },
  { id: 4, text: 'BizForm：無法自主實現業務流程控管，需由主系統串接 API／Webhook' },
  { id: 5, text: 'Vital KM × 合約管理：偏文件管理與表單整合，須整合 Vital CRM' },
  { id: 6, text: '點點簽 × 合約管理：可取代實體合約、具電子簽章法律效力，但無法單獨管理合約資料' },
];

const ROLE_CARDS: RoleCard[] = [
  { name: '點點簽',  accent: 'blue',   desc: '目前唯一提供電子簽章服務的方案。' },
  { name: 'BizForm', accent: 'orange', desc: '扮演 BPM 整合角色，讓 Vital CRM 串起業務流程節點。' },
  { name: 'Vital KM', accent: 'orange', desc: '扮演 CLM 整合角色，讓合約／文件集中管理。' },
];
...
function IconExclamation() {
  return (
    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
      <line x1="8" y1="3" x2="8" y2="9.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
...
function CellContent({ cell }: CellContentProps) {
  ...
  return (
    ...
    >
      <div style={{ color: iconColor }} className="relative flex items-center justify-center">
      ...
      {adopted && (
        <span className="text-[11px] font-medium" style={{ color: 'var(--orange-500)' }}>
          採用
        </span>
      )}
    </div>
...
export default function ModuleServiceSupportMatrix() {
  return (
    <div className="not-prose w-full space-y-6 overflow-x-auto">
      ...
      >
        合約管理整欄皆需整合：無單一服務能獨力涵蓋，故採「自建（管資料）＋ 點點簽（管簽章法律效力）」組合
      </p>
        ...
        <thead>
          <tr>
            ...
            {COLUMNS.map((col) => (
              ...
              >
                <div className="font-bold text-sm leading-tight">{col.label}</div>
                <div>
                  <span
      ...
      >
        ...
        <span className="flex items-center gap-1" style={{ color: 'var(--warning-500)' }}>
          <IconExclamation />
          <span style={{ color: 'var(--neutral-600)' }}>需整合或客制</span>
        </span>
        <span className="flex items-center gap-1" style={{ color: 'var(--neutral-400)' }}>
          <IconMinus />
      ...
      <div className="grid grid-cols-3 gap-3">
        {ROLE_CARDS.map((card) => (
          ...
          >
            <div className="font-bold text-sm" style={{ color: 'var(--neutral-800)' }}>
              {card.name}
            </div>
            <div
